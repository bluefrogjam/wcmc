...
class Runner extends CommandLineRunner with Logging {
  ...
  override def run(args: String*): Unit = {
    ...
    if (sampleName.isEmpty || method.isEmpty || mode.isEmpty) {
      ...
      this.process(Task(name = s"processing ${sampleName} with ${method}", email = submitter, acquisitionMethod = AcquisitionMethod.deserialize(method), samples = Seq(SampleToProcess(fileName = sampleName, matrix = Matrix("hp0", "human", "plasma", Seq.empty))), mode = mode, env = context.getEnvironment.getActiveProfiles.filter(p => Set("prod", "dev", "test").contains(p)).head))
    }
  }
