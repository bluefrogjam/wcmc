...
public class SpectralCentroiding {
	 ...
	 * @param spectra The first arg is the spectrum arrary collection. Each array, i.e. double[], should be [0]m/z and [1]intensity.
	...
	public static java.util.List<Ion> centroid(List<Ion> spectra, double massBin, boolean peakDetectionBasedCentroid) {
		if (peakDetectionBasedCentroid) {
			// peak detection based centroid
			ArrayList<Ion> centroidedSpectra = new ArrayList<>();
			...
			for (int i = 0; i < spectra.size(); i++) {
				...
				if (i >= spectra.size() - halfDatapoint) {
					firstDiffPeaklist.add(0.0);
					secondDiffPeaklist.add(0.0);
					continue;
				}
				...
				for (int j = 0; j < firstDiffCoeff.length; j++) {
					firstDiff += firstDiffCoeff[j] * spectra.get(i + j - halfDatapoint).intensity;
					secondDiff += secondDiffCoeff[j] * spectra.get(i + j - halfDatapoint).intensity;
				}
				...
				if (Math.abs(spectra.get(i).intensity - spectra.get(i - 1).intensity) > maxAmplitudeDiff) {
					maxAmplitudeDiff = Math.abs(spectra.get(i).intensity - spectra.get(i - 1).intensity);
				}
			}
			...
			for (int i = 2; i < spectra.size() - 2; i++) {
				if (Math.abs(spectra.get(i + 1).intensity() - spectra.get(i).intensity()) < amplitudeNoiseThresh &&
						Math.abs(spectra.get(i + 1).intensity() - spectra.get(i).intensity()) > 0) {
					amplitudeNoiseCandidate.add(Math.abs(spectra.get(i + 1).intensity() - spectra.get(i).intensity()));
				}
				if (Math.abs(firstDiffPeaklist.get(i)) < slopeNoiseThresh &&
						Math.abs(firstDiffPeaklist.get(i)) > 0) {
			...
			for (int i = 0; i < spectra.size(); i++) {
				if (i >= spectra.size() - 1 - minimumDatapointCriteria) {
					break;
				}
				//1. Left edge criteria
				if (firstDiffPeaklist.get(i) >= 0 && firstDiffPeaklist.get(i + 1) > slopeNoise * slopeNoiseFoldCriteria) {
					datapoints = new ArrayList<>();
					datapoints.add(new double[]{spectra.get(i).mass, spectra.get(i).intensity, firstDiffPeaklist.get(i), secondDiffPeaklist.get(i)});

					//2. Right edge criteria
					peaktopCheck = false;
					peaktopCheckPoint = i;
					while (true) {
						if (i + 1 == spectra.size() - 1) {
							break;
						}
						...
						datapoints.add(new double[]{spectra.get(i).mass(), spectra.get(i).intensity(), firstDiffPeaklist.get(i), secondDiffPeaklist.get(i)});
						if (!peaktopCheck &&
								firstDiffPeaklist.get(i - 1) > 0 && firstDiffPeaklist.get(i) < 0 &&
								secondDiffPeaklist.get(i) < -1 * peaktopNoise * peaktopNoiseFoldCriteria) {
							peaktopCheck = true;
							peaktopCheckPoint = i;
						}
						if (peaktopCheck &&
								peaktopCheckPoint + 2 <= i - 1 &&
					...
					detectedPeakInformation = new Ion(datapoints.get(peakTopId)[0], datapoints.get(peakTopId)[0]);
			...
			Collections.sort(centroidedSpectra, Comparator.comparing(Ion::intensity).reversed());
			double maxIntensity = centroidedSpectra.get(0).intensity;
			for (Ion centSpec : centroidedSpectra) {
			...
			Collections.sort(filteredCentroidedSpectra, Comparator.comparing(Ion::mass));
			...
			massBin = 0.1;
			ArrayList<Ion> centroidedSpectra = new ArrayList<>();
			double minMz = spectra.get(0).mass;
			double maxMz = spectra.get(spectra.size() - 1).mass();

			List<Ion> spectraList = spectra;
			double minInt = Collections.min(spectraList, Comparator.comparing(Ion::intensity)).intensity;
			Collections.sort(spectraList, Comparator.comparing(Ion::mass));
				...
				if (centroidedSpectra.size() != 0) {
					if (Math.abs(centroidedSpectra.get(centroidedSpectra.size() - 1).mass - sumXY / sumY) < massBin) {
						if (centroidedSpectra.get(centroidedSpectra.size() - 1).intensity < sumY) {
							centroidedSpectra.get(centroidedSpectra.size() - 1).mass = sumXY / sumY;
							centroidedSpectra.get(centroidedSpectra.size() - 1).intensity = sumY;
						}
					} else {
			...
			ArrayList<Ion> filteredCentroidedSpectra = new ArrayList<>();

			if (!centroidedSpectra.isEmpty()) {
				double maxIntensity = Collections.max(centroidedSpectra, Comparator.comparing(Ion::intensity).reversed()).intensity();
				for (Ion spectrum : centroidedSpectra) {
					if (spectrum.intensity() > maxIntensity * 0.001) {
						filteredCentroidedSpectra.add(spectrum);
					} else {
						break;
					}
				}
