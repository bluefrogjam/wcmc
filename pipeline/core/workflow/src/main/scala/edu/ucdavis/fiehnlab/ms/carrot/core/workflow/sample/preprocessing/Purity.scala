...
/**
  ...
  */
@Component
class PurityProcessing @Autowired()(val properties: WorkflowProperties, val puritySettings: PuritySettings) extends Process[Sample, ProcessedSample] with LazyLogging {
  ...
  def computePurity(spectra: MSSpectra, sample: Sample): Double = {
    ...
    val time = new IncludeByRetentionTimeWindow(spectra.retentionTimeInSeconds, puritySettings.retentionTimeRange)
    val mass = new IncludeByMassRange(spectra.basePeak.mass, puritySettings.massRange)
    val similiarity = new IncludeBySimilarity(spectra, puritySettings.minSimilarity)
...
@Component
@ConfigurationProperties(prefix = "purity")
class PuritySettings {
