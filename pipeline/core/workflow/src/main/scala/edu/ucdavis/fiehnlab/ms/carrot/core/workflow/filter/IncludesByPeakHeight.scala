...
import edu.ucdavis.fiehnlab.ms.carrot.core.api.types.sample.ms.{Feature, SimilaritySupport}

/**
  * Include the spectra, if any ion of it's ions has a mass between any of the required ions +/- the mass accuracy
  */
class IncludesByPeakHeight(val peaks: List[Ion], val massAccuracy: Double = 0.0005, val minIntensity: Float = 0.0f) extends Filter[Feature] {
  /**
