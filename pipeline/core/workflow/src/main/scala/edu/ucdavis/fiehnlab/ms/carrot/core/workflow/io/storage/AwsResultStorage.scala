...
import edu.ucdavis.fiehnlab.ms.carrot.cloud.bucket.BucketStorage
...
class AwsResultStorage @Autowired()(resourceStorage: ResourceStorage, writer: Writer[Sample]) extends ResultStorage with Logging {
  ...
  override def store(experiment: Experiment, task: Task): Unit = {
    ...
    experiment.classes.foreach { c =>
      c.samples.foreach { s =>
        writer.write(out, s)
        out.flush()
        resourceStorage match {
          case bs: BucketStorage =>
