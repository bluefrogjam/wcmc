...
class LCMSTargetAnnotationProcessTest extends WordSpec with Logging {
  ...
  "LCMSTargetAnnotationProcessTest" should {
    ...
    correctedSample.foreach { sample =>
      s"process ${sample} without recursive annotation and using gaussian similarity" ignore {
        ...
        targetValues(sample.name).foreach(tgt => {
          logger.info(s"target: ${
            quantified.featuresUsedForCorrection.filter(_.target.name.get == tgt._1)
                .map(f => s"${f.annotation.retentionTimeInSeconds} (${f.annotation.massOfDetectedFeature})").mkString("; ")
          }")
          quantified.featuresUsedForCorrection.filter(_.target.name.get == tgt._1).head.annotation.retentionTimeInSeconds shouldBe tgt._2 +- annotation.lcmsProperties.retentionIndexWindow
        })
      ...
      s"process ${sample} with recursive annotation and using gaussian similarity" ignore {
        ...
        //these are our ISTD being compared to the annotated features
        targetValues(sample.name).foreach(tgt => {
          logger.info(s"target: ${tgt._1} = ${tgt._2}")
          quantified.spectra.foreach { f =>
            if (f.target.name.get == tgt._1)
              logger.info(s"feature: ${f}")
          }
          //          feature.annotation.retentionTimeInSeconds shouldBe tgt._2 +- annotation.lcmsProperties.retentionIndexWindow
