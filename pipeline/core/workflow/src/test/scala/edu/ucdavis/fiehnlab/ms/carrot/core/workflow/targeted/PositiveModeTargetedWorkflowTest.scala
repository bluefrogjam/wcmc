...
/**
  ...
  */
abstract class PositiveModeTargetedWorkflowTest extends WordSpec with Matchers with LazyLogging {
  ...
  /**
    * provides us with access to the experiment definition
    *
    * @return
    */
  def experimentDefinition: InputStream


  val expectedCorrelation: Double = 0.99

  // expected results constants please override in each test
  def expectedContentSize(): Integer
  def expectedValidationVal(): Double
  ...
  "LCMSPositiveModeTargetWorkflowTest" must {
    ...
    "process an experiment" when {
      ...
      "export all our data" should {
        ...
        "workflow has test listener assigned" in {
        ...
        "provide validation" when {
          ...
          s"*007 1_CUDA ISTD [M+H]+ regression coefficient > $expectedCorrelation" in {
            ...
            val refrenceTarges: Seq[Double] = Array(667364.1,655438.8,630141.3,657902.9,641507.9,660547.9,650333.6,658413.1,627398.6,
              653240.2,738846.3,719581.7,729094.6,743863.3,799186.8,719726.9,671941.6,729776.1,709123,741461.8,635910,639902.7,620975.3,
              618259.1,652854.4,628877.3,613902.5,618652.3,623229.6
            )
          ...
          s"*015 1_MG 17:0/0:0/0:0 [M+Na]+ ISTD regression coefficient > ${expectedCorrelation}" in {
            ...
            //manually confirmed values by tomas
            val refrenceTarges: Seq[Double] = Array(84172.55,88753.84,91590.15,86160.72,89132,83534.53,90063.11,88874.77,
	            91844.45,91814.16,95505.53,94563.38,96612.41,90806.9,93718.09,88924,95578.27,91195.52,94071.31,91879.78,
	            91126.63,90971.86,93368.55,87103.61,90979.8,84053.64,89646.35,86476.57,88973.58
            )
          ...
          s"*020 1_Sphingosine d17:1 [M+H]+ ISTD regression coefficient > ${expectedCorrelation}" in {
            ...
            //manually confirmed values by tomas
            val refrenceTarges: Seq[Double] = Array(65737.89,64059.59,64284.1,61659.55,65604.73,62722.94,63476.96,64017.91,62023.86,
              65716.41,71173.5,68446.19,70555.9,72480.45,75297.94,66608.14,67781.21,71287.27,70727.13,75156.08,50854.79,
              48742.45,51324.99,50445.16,54343.21,48873.45,49486.82,50665.44,50382.46
            )
          ...
          s"*010 1_DG (18:1/2:0/0:0) [M+Na]+ ISTD regression coefficient > ${expectedCorrelation}" in {
            ...
            //manually confirmed values by tomas
            val refrenceTarges: Seq[Double] = Array(1144086,1150598,1146070,1134526,1160032,1089979,1118873,1091180,1124240,
              1130791,1156351,1257627,1228662,1168048,1288668,1178555,1135616,1171308,1149503,1174480,994138.8,988540.3,1034452,
              1051443,1051769,1056460,1041417,1046814,1016227
            )
...
@Component
class TestWorkflowEventListener extends WorkflowEventListener with LazyLogging {
  var quantifiedExperiment: Experiment = null
  ...
  override def handle(workflowEvent: WorkflowEvent): Unit = workflowEvent match {
	  case event: QuantificationFinishedEvent => {
		  logger.debug(" === Quant Finished !!! ===")
