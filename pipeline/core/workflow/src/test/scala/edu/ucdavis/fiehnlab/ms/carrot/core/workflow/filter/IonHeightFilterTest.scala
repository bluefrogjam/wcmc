...
class IonHeightFilterTest extends WordSpec with Matchers with Logging {
  @Value("${carrot.filters.minIntensity:1000}")
  val intensity: Int = 0

  @Autowired
  val intFilter: Filter[Target] = null
  ...
  "TargetIonHeightFilterTest" should {
    ...
    "have minIntensity defined" in {
      ...
      intFilter match {
        case f: IonHeightFilter =>
          f.minIntensity should equal(intensity)
      }
    }
